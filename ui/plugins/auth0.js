  ...
  instance = new Vue({
    ...
    methods: {
      ...
      loginWithRedirect (o) {
        ...
        sessionStorage.setItem(ORIGINAL_URL_KEY, redirectTo)
...
export default function ({ redirect }) {
  ...
  if (domain && clientId && process.client) {
    Vue.use(Auth0Plugin, {
      domain,
      clientId,
      onRedirectCallback () {
        const toUrl = sessionStorage.getItem(ORIGINAL_URL_KEY) || window.location.pathname
        if (toUrl !== '/') {
          redirect(toUrl)
        } else {
          // TODO: allow index.vue to determine default route
          redirect('/')
        }
      }
    })
